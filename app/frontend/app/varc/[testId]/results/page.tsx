...
function VarcResultsInner() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await apiFetch(`/api/varc/sessions/${sid}`);
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f4f6fa', color: '#222', fontFamily: 'Arial, Helvetica, sans-serif' }}>
      ...
      <div style={{ background: '#003366', color: 'white', padding: '12px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ fontWeight: 'bold', fontSize: 15 }}>VARC Section Test {testId} — Results</div>
        <div style={{ display: 'flex', gap: 10 }}>
          <button onClick={() => router.push('/varc')} style={{ padding: '6px 16px', background: 'rgba(255,255,255,0.15)', color: 'white', border: '1px solid rgba(255,255,255,0.3)', borderRadius: 4, cursor: 'pointer', fontSize: 13 }}>All Tests</button>
        </div>
      </div>
          ...
          <div style={{ background: 'white', borderRadius: 8, padding: '24px 28px', boxShadow: '0 1px 4px rgba(0,0,0,0.08)' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12 }}>
              {[
                ...
                { label: 'Unattempted', val: 34 - score.attempted, color: '#888' },
              ].map(s => (
...
export default function VarcResultsPage() {
  return (
