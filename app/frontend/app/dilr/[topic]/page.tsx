...
interface Q {
  q_slug: string; order: number; question_text: string;
...
interface SetT {
  ...
  title: string; setup: string; source: string; questions: Q[];
}
...
export default function DilrSetPlayer() {
  ...
  // flatten sets -> items, restricted to the chosen set
  const items = useMemo<Item[]>(() => {
    const out: Item[] = [];
    sets.filter(s => s.set_slug === activeSet).forEach(s => (s.questions || []).forEach((q, k) => {
  ...
  useEffect(() => {
    const h = (e: KeyboardEvent) => {
      if (done || !item) return;
      if (item.answer_format === 'typed') {
        if (e.key === 'Enter' && !revealed && selected) reveal(selected);
        else if (e.key === 'Enter' && revealed) goNext();
        return;
      }
  ...
  const backToSets = () => { setActiveSet(null); setDone(false); setI(0); };

  // ---------- SET CHOOSER ----------
  if (!activeSet) {
    const sorted = [...sets].sort((a, b) => a.order - b.order);
    return (
      <Shell accent={acc} name={topic.name} onBack={() => router.push('/dilr')}>
        <div style={{ maxWidth: 720, margin: '0 auto', padding: '24px 20px 60px' }}>
          <h2 style={{ fontSize: 20, fontWeight: 800, margin: '0 0 4px' }}>{topic.name}</h2>
          <p style={{ fontSize: 13.5, color: '#666', margin: '0 0 20px' }}>Pick a set to solve. Untimed, and your progress is saved as you go.</p>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {sorted.map((s, k) => {
              const total = s.questions.length;
              const attempted = s.questions.filter(q => q.attempt).length;
              const correct = s.questions.filter(q => q.attempt?.correct).length;
              const diffC = DIFF_COLOR[s.difficulty] || '#777';
              return (
                <div key={s.set_slug}
                  onClick={() => { setActiveSet(s.set_slug); setI(0); setDone(false); }}
                  style={{
                    background: '#fff', border: '1px solid #e4e4e4', borderRadius: 12, padding: '14px 18px',
                    cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 14, boxShadow: '0 1px 2px rgba(0,0,0,0.04)',
                  }}
                  onMouseEnter={e => { e.currentTarget.style.boxShadow = '0 4px 14px rgba(0,0,0,0.08)'; }}
                  onMouseLeave={e => { e.currentTarget.style.boxShadow = '0 1px 2px rgba(0,0,0,0.04)'; }}
                >
                  <div style={{ flex: '0 0 auto', width: 34, height: 34, borderRadius: 9, background: `${acc}18`, color: acc, fontWeight: 800, fontSize: 13, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>{k + 1}</div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      <span style={{ fontWeight: 700, fontSize: 14.5 }}>Set {k + 1}: {s.title}</span>
                      <span style={{ fontSize: 10.5, fontWeight: 700, color: diffC, background: `${diffC}18`, padding: '2px 8px', borderRadius: 99 }}>{s.difficulty}</span>
                    </div>
                    <div style={{ fontSize: 12, color: '#888', marginTop: 3 }}>{s.pattern_type} · {total} questions{attempted > 0 ? ` · ${attempted}/${total} attempted` : ''}{attempted > 0 ? ` · ${correct} correct` : ''}</div>
                  </div>
  ...
  const attemptedCount = Object.keys(results).filter(k => items.some(it => it.q_slug === k)).length;
  const correctCount = items.filter(it => results[it.q_slug]?.correct).length;

  // ---------- SUMMARY ----------
  if (done) {
    const accuracy = attemptedCount ? Math.round((correctCount / attemptedCount) * 100) : 0;
    return (
      <Shell accent={acc} name={topic.name} onBack={backToSets} backLabel="← Sets">
        <div style={{ maxWidth: 560, margin: '40px auto', textAlign: 'center' }}>
          <div style={{ fontSize: 40 }}>🎯</div>
          <h2 style={{ fontSize: 24, fontWeight: 800, margin: '8px 0 4px' }}>Set complete</h2>
          ...
          <div style={{ display: 'flex', gap: 10, justifyContent: 'center' }}>
            ...
            <button onClick={backToSets} style={btnGhost}>Back to sets</button>
          </div>
    ...
    <Shell accent={acc} name={topic.name} onBack={backToSets} backLabel="← Sets">
      ...
      <div style={{ maxWidth: 720, margin: '0 auto', padding: '18px 20px 60px' }}>
        ...
        <div style={{ background: '#fff', border: '1px solid #e4e4e4', borderRadius: 14, padding: '20px 22px', boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
          ...
          {/* options or typed answer */}
          {item.answer_format === 'typed' ? (
            <input
              type="text"
              inputMode="decimal"
              value={selected ?? ''}
              disabled={revealed}
              onChange={e => { const v = e.target.value; if (/^-?[0-9]*\.?[0-9]*$/.test(v)) setSelected(v || null); }}
              placeholder="Type your numeric answer"
              style={{
                width: 240, padding: '10px 14px', fontSize: 15, borderRadius: 8,
                border: `1.5px solid ${revealed ? (selected === item.correct_answer ? '#228B22' : '#C0392B') : '#e0e2e6'}`,
                outline: 'none', background: revealed ? (selected === item.correct_answer ? '#f0f9f0' : '#fbeeec') : '#fff',
              }}
            />
          ) : (
...
function Shell({ children, accent = '#0891B2', name, onBack, backLabel = '← Topics' }: { children: React.ReactNode; accent?: string; name?: string; onBack?: () => void; backLabel?: string; }) {
  return (
    <div style={{ minHeight: '100vh', background: '#FAFAFA', fontFamily: F, color: '#222' }}>
      <div style={{ background: '#121212', color: '#fff', padding: '13px 24px', display: 'flex', alignItems: 'center', gap: 12 }}>
        <div onClick={onBack} style={{ fontSize: 13, color: '#bbb', cursor: 'pointer' }}>{backLabel}</div>
        <div style={{ width: 8, height: 8, borderRadius: 99, background: accent }} />
        <div style={{ fontWeight: 700, fontSize: 14.5 }}>{name || 'LR Topics'}</div>
      </div>
      {children}
    </div>
  );
