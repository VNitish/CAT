...
const DEFAULT_TIME = 55 * 60; // placeholder until the server returns the real per-test time
...
// time_spent is whole seconds on the wire; the exam page accumulates in ms
// internally so that repeated short visits to a question don't round away.
interface QState { status: QStatus; answer: string | null; time_spent?: number }
...
function QtExamInner() {
  ...
  // Kept in refs rather than state: the clock ticks every second and putting it
  // in React state would re-render (and re-trigger the debounced save) on every
  // tick. Instead we bank elapsed milliseconds against the question being left,
  // and fold the totals into the payload only at save/submit time.
  const timeSpentRef = useRef<Record<string, number>>({}); // qid -> milliseconds
  const activeQidRef = useRef<string | null>(null);
  const activeSinceRef = useRef<number>(Date.now());

  // Credit everything since the last flush to whichever question is currently open.
  const flushActiveTime = useCallback(() => {
    const now = Date.now();
    const qid = activeQidRef.current;
    if (qid) {
      timeSpentRef.current[qid] = (timeSpentRef.current[qid] || 0) + (now - activeSinceRef.current);
    }
    activeSinceRef.current = now;
  }, []);

  // Stamp the banked per-question seconds onto a question_states map for the wire.
  const withTimes = useCallback((base: Record<string, QState>) => {
    flushActiveTime();
    const out: Record<string, QState> = {};
    for (const [qid, st] of Object.entries(base)) {
      out[qid] = { ...st, time_spent: Math.round((timeSpentRef.current[qid] || 0) / 1000) };
    }
    return out;
  }, [flushActiveTime]);
  ...
  useEffect(() => {
    ...
    if (!(Number.isInteger(test) && test >= 1)) { setError('Invalid test'); setLoading(false); return; }
    (async () => {
      try {
        const res = await apiFetch('/api/qt/start', {
          method: 'POST',
        ...
        // Resume: re-bank the seconds already logged against each question so a
        // Save & Exit round-trip doesn't reset the per-question clocks.
        qs.forEach(q => {
          const prior = states[q._id]?.time_spent;
          if (typeof prior === 'number' && prior > 0) timeSpentRef.current[q._id] = prior * 1000;
        });
  ...
  // ── Timer ──
  // Counts down and then straight on into negative numbers. The test is never
  // force-closed when the clock hits zero: the candidate finishes the paper and
  // submits, and the overrun is recorded on the results page.
  useEffect(() => {
  ...
  const persist = useCallback(async () => {
    ...
    setSaveState('saving');
    try {
      const res = await apiFetch(`/api/qt/${sessionId}/save`, {
        method: 'POST',
        body: JSON.stringify({ question_states: withTimes(qStatesRef.current), time_left: timeLeftRef.current }),
      });
  ...
  useEffect(() => {
    if (!q) return;
    // Bank the time against the question we are leaving, then start the clock
    // on the one we just opened.
    flushActiveTime();
    activeQidRef.current = q._id;
    setPendingAnswer(qStates[q._id]?.answer ?? null);
    setQStates(prev => {
  ...
  const commit = useCallback((markForReview: boolean, advance: boolean) => {
    if (!q) return;
    // A TITA box holding only whitespace counts as unanswered.
    const ans = pendingAnswer && pendingAnswer.trim() ? pendingAnswer.trim() : null;
  ...
  const saveAndExit = useCallback(async () => {
    commit(false, false);
    setTimeout(async () => { await persist(); router.push('/quant-tests'); }, 100);
  ...
  const doSubmit = useCallback(async () => {
    ...
    if (q) {
      ...
      const ans = pendingAnswer && pendingAnswer.trim() ? pendingAnswer.trim() : null;
      const status: QStatus = ans
        ? (existing?.status === 'marked' || existing?.status === 'answered-marked' ? 'answered-marked' : 'answered')
        : (existing?.status || 'not-answered');
      finalStates[q._id] = { status, answer: ans };
    }
    try {
      const res = await apiFetch(`/api/qt/${sessionId}/submit`, {
        method: 'POST',
        body: JSON.stringify({ question_states: withTimes(finalStates), time_left: timeLeftRef.current }),
      });
      if (!res.ok) throw new Error('submit failed');
      router.push(`/quant-tests/${testId}/results?sid=${sessionId}`);
    } catch {
      setSubmitting(false);
      submittedRef.current = false;
      setError('Submission failed. Please try again.');
    }
  }, [sessionId, submitting, q, pendingAnswer, router, testId, withTimes]);
  ...
  if (error) {
    return (
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#e8e8e8', color: '#cc3300', flexDirection: 'column', gap: 16 }}>
        ...
        <button onClick={() => router.push('/quant-tests')} style={{ padding: '8px 20px', background: '#003366', color: 'white', border: 'none', borderRadius: 4, cursor: 'pointer' }}>Back to Quant Tests</button>
      </div>
    );
  }
  if (!q) return null;
    ...
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', background: '#d6d6d6', color: '#111111', fontFamily: 'Arial, Helvetica, sans-serif' }}>
      ...
      <div style={{ background: '#003366', color: 'white', padding: '0 12px', display: 'flex', alignItems: 'stretch', borderBottom: '2px solid #cc6600', minHeight: 44, flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, paddingRight: 16, borderRight: '1px solid rgba(255,255,255,0.2)' }}>
          ...
          <div style={{ fontSize: 12, fontWeight: 'bold', lineHeight: 1.2 }}>Quant Mock<br /><span style={{ color: '#aac4ff', fontSize: 10, fontWeight: 'normal' }}>{block ? `${block} · ` : ''}Test {testId}</span></div>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', flex: 1, paddingLeft: 16 }}>
          <div style={{ padding: '0 16px', background: '#cc6600', display: 'flex', alignItems: 'center', height: '100%', fontSize: 13, fontWeight: 'bold' }}>Quantitative Ability</div>
        </div>
        ...
        {/* Timer — runs into negative time rather than ending the test */}
        <div style={{
          display: 'flex', alignItems: 'center', gap: 12, padding: '0 12px',
          background: overtime ? '#a01e00' : 'transparent',
        }}>
          <div style={{ textAlign: 'right' }}>
            <div style={{ fontSize: 10, color: overtime ? '#ffd9cc' : '#aac4ff', fontWeight: overtime ? 'bold' : 'normal', letterSpacing: overtime ? 0.5 : 0 }}>
              {overtime ? 'OVERTIME' : 'Time Left'}
            </div>
            <div style={{ fontSize: 18, fontWeight: 'bold', color: 'white', fontVariantNumeric: 'tabular-nums', letterSpacing: 1 }}>
              {formatTime(timeLeft)}
            </div>
          </div>
        </div>
      ...
      {/* Overtime banner */}
      {overtime && (
        <div style={{ background: '#fff1ee', borderBottom: '1px solid #f5c6ba', color: '#a01e00', fontSize: 12, padding: '5px 16px', textAlign: 'center', flexShrink: 0 }}>
          You are <strong>{formatTime(timeLeft).replace('-', '')}</strong> past the 55-minute limit. The test stays open &mdash; finish it, but note the overrun in your review.
        </div>
      )}
      ...
      <div style={{ background: '#e8f0fe', borderBottom: '1px solid #c5d5f5', padding: '4px 16px', fontSize: 11, color: '#333', display: 'flex', gap: 20, alignItems: 'center', flexShrink: 0 }}>
        ...
        <span>|</span>
        <span>Negative Marks: <strong style={{ color: q.marks_incorrect === 0 ? '#228B22' : '#cc3300' }}>{q.marks_incorrect}</strong></span>
        <span style={{ background: isTita ? '#fff2e0' : '#eef2ff', border: `1px solid ${isTita ? '#f0c48a' : '#c9d6ff'}`, color: isTita ? '#96590a' : '#2a3f9d', padding: '1px 7px', borderRadius: 3, fontWeight: 'bold' }}>
          {q.question_type}
        </span>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', minWidth: 0 }}>
          <div style={{ flex: 1, overflow: 'auto', padding: '12px 16px', background: 'white' }}>
            ...
            <div style={{ marginBottom: 20 }}>
              ...
              <div style={{ fontSize: 14, lineHeight: 1.8, color: '#111', marginBottom: 18 }}><MathText>{q.question_text}</MathText></div>
              ...
              {isTita && (
                <div style={{ maxWidth: 420 }}>
                  <label htmlFor="tita" style={{ display: 'block', fontSize: 12, fontWeight: 'bold', color: '#003366', marginBottom: 6 }}>
                    Your answer
                  </label>
                  <input
                    id="tita"
                    ...
                    value={pendingAnswer ?? ''}
                    onChange={e => {
                      // Digits, one leading minus and one decimal point only.
                      const v = e.target.value.replace(/[^0-9.\-]/g, '');
                      setPendingAnswer(v);
                    }}
                    placeholder="Type a number"
                    style={{
                      width: '100%', padding: '12px 14px', fontSize: 18, fontWeight: 'bold',
                      color: '#003366', border: '2px solid #003399', borderRadius: 4,
                      background: '#f6f9ff', fontVariantNumeric: 'tabular-nums', letterSpacing: 0.5,
                    }}
                  />
                  <div style={{ fontSize: 11.5, color: '#666', marginTop: 8, lineHeight: 1.6 }}>
                    Type in the answer &mdash; there are no options for this question.
                    <br />
                    <strong style={{ color: '#228B22' }}>No negative marking</strong>, so never leave it blank. Click <strong>Save &amp; Next</strong> to record it.
                  </div>
                </div>
              )}
            </div>
        ...
        {!paletteCollapsed ? (
          <div style={{ width: 220, background: '#e8e8e8', borderLeft: '2px solid #ccc', display: 'flex', flexDirection: 'column', flexShrink: 0, overflow: 'hidden' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '6px 10px', background: '#003366', color: 'white' }}>
              <span style={{ fontSize: 12, fontWeight: 'bold' }}>Quantitative Ability</span>
                ...
                {questions.map((qq, idx) => {
                  ...
                  return (
                    <button
                      ...
                      title={`${qq.topic} · ${qq.question_type}`}
                      style={{
      ...
      {showSubmitConfirm && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 2000 }}>
          <div style={{ background: 'white', borderRadius: 8, padding: 28, maxWidth: 460, width: '90%', boxShadow: '0 8px 32px rgba(0,0,0,0.3)' }}>
            ...
            <p style={{ fontSize: 13, color: '#555', margin: '0 0 16px' }}>You are about to submit Quant Test {testId}. This cannot be undone.</p>
            <div style={{ background: '#f5f5f5', borderRadius: 4, padding: '10px 14px', fontSize: 12, marginBottom: 16 }}>
              ...
              <div style={{ marginTop: 6, paddingTop: 6, borderTop: '1px solid #e0e0e0' }}>
                {overtime
                  ? <span style={{ color: '#a01e00' }}>Overtime: <strong>{formatTime(timeLeft).replace('-', '')}</strong> past the limit</span>
...
export default function QtExamPage() {
  return (
    <Suspense fallback={
