...
// Compact form for per-question chips: "48s", "2m 05s".
...
// Roll questions up by an arbitrary key (topic / pattern / difficulty).
...
function QtResultsInner() {
  ...
  const [duration, setDuration] = useState<number>(55 * 60);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await apiFetch(`/api/qt/sessions/${sid}`);
        ...
        setDuration(data.duration_seconds || 55 * 60);
  ...
  // ── Time analysis ──
  // Split the clock by outcome. The number that actually matters for CAT is how
  // much time went into questions that earned nothing.
  const correctQs = questions.filter(q => q.user_answer && q.is_correct);
  const wrongQs   = questions.filter(q => q.user_answer && !q.is_correct);
  // "Read it and walked away" — visited, timed, but left blank.
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f4f6fa', color: '#222', fontFamily: 'Arial, Helvetica, sans-serif' }}>
      ...
      <div style={{ background: '#003366', color: 'white', padding: '12px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ fontWeight: 'bold', fontSize: 15 }}>Quantitative Ability {block ? `· ${block}` : ''} — Test {testId} Results</div>
        <div style={{ display: 'flex', gap: 10 }}>
          <button onClick={() => router.push('/quant-tests')} style={{ padding: '6px 16px', background: 'rgba(255,255,255,0.15)', color: 'white', border: '1px solid rgba(255,255,255,0.3)', borderRadius: 4, cursor: 'pointer', fontSize: 13 }}>All Tests</button>
        </div>
      </div>
      ...
      {score && (
        <div style={{ maxWidth: 880, margin: '24px auto 0', padding: '0 20px' }}>
          <div style={{ background: 'white', borderRadius: 8, padding: '24px 28px', boxShadow: '0 1px 4px rgba(0,0,0,0.08)' }}>
            ...
            {/* Time taken vs the 55-minute limit */}
            <div style={{
      ...
      <div style={{ maxWidth: 880, margin: '20px auto 0', padding: '0 20px' }}>
        <div style={{ background: 'white', borderRadius: 8, padding: '20px 24px', boxShadow: '0 1px 4px rgba(0,0,0,0.08)' }}>
          <h2 style={{ fontSize: 14, color: '#003366', margin: '0 0 14px' }}>By chapter</h2>
          {byTopic.map(t => {
      ...
      {hasTiming && (
        <div style={{ maxWidth: 880, margin: '20px auto 0', padding: '0 20px' }}>
          <div style={{ background: 'white', borderRadius: 8, padding: '20px 24px', boxShadow: '0 1px 4px rgba(0,0,0,0.08)' }}>
            <h2 style={{ fontSize: 14, color: '#003366', margin: '0 0 6px' }}>Where your time went</h2>
            ...
            {/* Stacked bar: productive vs wasted */}
            ...
            {/* Slowest questions */}
            <div style={{ fontSize: 12, fontWeight: 700, color: '#003366', marginBottom: 8 }}>Longest single questions</div>
            {slowest.map(q => {
              ...
              return (
                <div key={q._id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '6px 0', borderBottom: '1px solid #f2f4f7', fontSize: 12.5 }}>
                  <span style={{ fontWeight: 700, color: '#003366', width: 34, flexShrink: 0 }}>Q{q.question_number}</span>
      ...
      {weakPatterns.length > 0 && (
        <div style={{ maxWidth: 880, margin: '20px auto 0', padding: '0 20px' }}>
          <div style={{ background: 'white', borderRadius: 8, padding: '20px 24px', boxShadow: '0 1px 4px rgba(0,0,0,0.08)' }}>
            <h2 style={{ fontSize: 14, color: '#003366', margin: '0 0 6px' }}>Patterns to revisit</h2>
      ...
      <div style={{ maxWidth: 880, margin: '24px auto', padding: '0 20px 60px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
          <h2 style={{ fontSize: 16, color: '#003366', margin: 0 }}>Question Review</h2>
            ...
            {([
              ...
              <button
                ...
                style={{
                  ...
                  background: filter === key ? '#003366' : 'white',
                  color: filter === key ? 'white' : '#555',
                  border: `1px solid ${filter === key ? '#003366' : '#d6dbe4'}`,
                }}
              >
                {label}
              </button>
          ...
          return (
            <div key={q._id} style={{ background: 'white', border: `1px solid ${borderColor}`, borderRadius: 8, padding: '16px 20px', marginBottom: 12 }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 10, fontSize: 10.5 }}>
                <span style={{ background: '#eef4ff', border: '1px solid #d3e0f7', color: '#2a4f8f', fontWeight: 600, padding: '2px 7px', borderRadius: 3 }}>{q.pattern}</span>
              ...
              {q.explanation && q.explanation.trim() && (
                <div style={{ marginTop: 12, background: '#f4f8ff', border: '1px solid #d6e4ff', borderRadius: 4, padding: '10px 14px', fontSize: 13, lineHeight: 1.7, color: '#234' }}>
                  <div style={{ fontWeight: 700, fontSize: 11, color: '#003366', marginBottom: 4, textTransform: 'uppercase', letterSpacing: 0.5 }}>Solution</div>
...
export default function QtResultsPage() {
  return (
