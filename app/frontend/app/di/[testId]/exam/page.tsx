...
function DiExamInner() {
  ...
  useEffect(() => {
    ...
    if (!(Number.isInteger(test) && test >= 1)) { setError('Invalid set'); setLoading(false); return; }
    (async () => {
      try {
        const res = await apiFetch('/api/di/start', {
          method: 'POST',
  ...
  const persist = useCallback(async () => {
    ...
    setSaveState('saving');
    try {
      const res = await apiFetch(`/api/di/${sessionId}/save`, {
        method: 'POST',
  ...
  const saveAndExit = useCallback(async () => {
    commit(false, false);
    setTimeout(async () => { await persist(); router.push('/di'); }, 100);
  ...
  const doSubmit = useCallback(async () => {
    ...
    try {
      const res = await apiFetch(`/api/di/${sessionId}/submit`, {
        method: 'POST',
        body: JSON.stringify({ question_states: finalStates, time_left: timeLeftRef.current }),
      });
      if (!res.ok) throw new Error('submit failed');
      router.push(`/di/${testId}/results?sid=${sessionId}`);
    } catch {
      setSubmitting(false);
  ...
  if (error) {
    return (
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#e8e8e8', color: '#cc3300', flexDirection: 'column', gap: 16 }}>
        ...
        <button onClick={() => router.push('/di')} style={{ padding: '8px 20px', background: '#003366', color: 'white', border: 'none', borderRadius: 4, cursor: 'pointer' }}>Back to DI</button>
      </div>
    );
  }
  if (!q) return null;
    ...
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', background: '#d6d6d6', color: '#111111', fontFamily: 'Arial, Helvetica, sans-serif' }}>
      ...
      <div style={{ background: '#003366', color: 'white', padding: '0 12px', display: 'flex', alignItems: 'stretch', borderBottom: '2px solid #cc6600', minHeight: 44, flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, paddingRight: 16, borderRight: '1px solid rgba(255,255,255,0.2)' }}>
          ...
          <div style={{ fontSize: 12, fontWeight: 'bold', lineHeight: 1.2 }}>DI Mock<br /><span style={{ color: '#aac4ff', fontSize: 10, fontWeight: 'normal' }}>{tier ? `${tier} · ` : ''}Set {testId}</span></div>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', flex: 1, paddingLeft: 16 }}>
          <div style={{ padding: '0 16px', background: '#cc6600', display: 'flex', alignItems: 'center', height: '100%', fontSize: 13, fontWeight: 'bold' }}>Data Interpretation</div>
        </div>
          ...
          <div style={{ flex: 1, overflow: 'auto', padding: '12px 16px', background: 'white' }}>
            ...
            {passage && (
              <div style={{ background: '#f9f9f9', border: '1px solid #ddd', borderRadius: 4, padding: '12px 16px', marginBottom: 16, fontSize: 13, lineHeight: 1.8, color: '#222', maxHeight: 520, overflow: 'auto' }}>
                <div style={{ fontWeight: 'bold', fontSize: 11, color: '#003366', marginBottom: 8, textTransform: 'uppercase', letterSpacing: 0.5 }}>Data Set</div>
                {/* Rendered through MathText so the cropped chart/table image (and any markdown/math) displays */}
        ...
        {!paletteCollapsed ? (
          <div style={{ width: 220, background: '#e8e8e8', borderLeft: '2px solid #ccc', display: 'flex', flexDirection: 'column', flexShrink: 0, overflow: 'hidden' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '6px 10px', background: '#003366', color: 'white' }}>
              <span style={{ fontSize: 12, fontWeight: 'bold' }}>Data Interpretation</span>
      ...
      {showSubmitConfirm && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 2000 }}>
          <div style={{ background: 'white', borderRadius: 8, padding: 28, maxWidth: 460, width: '90%', boxShadow: '0 8px 32px rgba(0,0,0,0.3)' }}>
            ...
            <p style={{ fontSize: 13, color: '#555', margin: '0 0 16px' }}>You are about to submit Data Interpretation Set {testId}. This cannot be undone.</p>
...
export default function DiExamPage() {
  return (
    <Suspense fallback={
