...
function DiResultsInner() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await apiFetch(`/api/di/sessions/${sid}`);
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f4f6fa', color: '#222', fontFamily: 'Arial, Helvetica, sans-serif' }}>
      ...
      <div style={{ background: '#003366', color: 'white', padding: '12px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ fontWeight: 'bold', fontSize: 15 }}>Data Interpretation {tier ? `· ${tier}` : ''} — Set {testId} Results</div>
        <div style={{ display: 'flex', gap: 10 }}>
          <button onClick={() => router.push('/di')} style={{ padding: '6px 16px', background: 'rgba(255,255,255,0.15)', color: 'white', border: '1px solid rgba(255,255,255,0.3)', borderRadius: 4, cursor: 'pointer', fontSize: 13 }}>All Sets</button>
        </div>
      </div>
            ...
            <div key={q._id} style={{ background: 'white', border: `1px solid ${borderColor}`, borderRadius: 8, padding: '16px 20px', marginBottom: 12 }}>
              ...
              {q.context_passage && (
                <details style={{ marginBottom: 10 }}>
                  <summary style={{ fontSize: 12, color: '#666', cursor: 'pointer' }}>Show data set</summary>
                  <div style={{ background: '#f9f9f9', border: '1px solid #eee', borderRadius: 4, padding: '10px 12px', marginTop: 6, fontSize: 12.5, lineHeight: 1.7, color: '#333' }}>
                    <MathText>{q.context_passage}</MathText>
                  </div>
                </details>
              )}

              <div style={{ fontSize: 14, lineHeight: 1.6, color: '#111', marginBottom: 12 }}><MathText>{q.question_text}</MathText></div>
...
export default function DiResultsPage() {
  return (
