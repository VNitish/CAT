...
interface DilrTest { test: number; label: string; questions: number; duration_minutes: number; marks: number; }

function DilrtInstructionsInner() {
  const router = useRouter();
  const params = useParams();
  const testId = params.testId as string;
  const [agreed, setAgreed] = useState(false);
  const [meta, setMeta] = useState<DilrTest | null>(null);

  useEffect(() => {
    fetch('/api/dilr-tests')
      .then(r => r.json())
      .then(d => { if (Array.isArray(d)) setMeta(d.find((t: DilrTest) => String(t.test) === testId) || null); })
      .catch(() => {});
  }, [testId]);
  ...
  const startExam = () => {
    ...
    router.push(`/dilr-tests/${testId}/exam`);
  };

  return (
    <div style={{ minHeight: '100vh', background: '#e8e8e8', color: '#111111', display: 'flex', flexDirection: 'column' }}>
      {/* Top bar */}
      <div style={{ background: '#312e81', color: 'white', padding: '8px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: '2px solid #4F46E5' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <div style={{ width: 36, height: 36, background: '#4F46E5', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 'bold', fontSize: 14 }}>C</div>
          <div>
            <div style={{ fontWeight: 'bold', fontSize: 14 }}>LR Sample Test Portal</div>
            <div style={{ fontSize: 11, color: '#c7c9f5' }}>Logical Reasoning</div>
          </div>
        </div>
        <div style={{ fontSize: 13, color: '#c7c9f5' }}>{meta ? meta.label : `Test ${testId}`}</div>
      </div>

      {/* Title bar */}
      <div style={{ background: '#4338ca', padding: '10px 16px', textAlign: 'center' }}>
        <div style={{ color: 'white', fontWeight: 'bold', fontSize: 16 }}>Logical Reasoning — {meta ? meta.label : `Test ${testId}`}</div>
        <div style={{ color: '#dcdcff', fontSize: 12 }}>Please read all instructions carefully before starting the test</div>
      </div>
      ...
      <div style={{ flex: 1, overflow: 'auto', padding: '16px' }}>
        <div style={{ maxWidth: 900, margin: '0 auto' }}>
          ...
          <div style={{ background: 'white', padding: '20px 24px', border: '1px solid #ccc', fontSize: 13, lineHeight: 1.7 }}>
            ...
            <ol style={{ paddingLeft: 24, margin: 0 }}>
              <li style={{ marginBottom: 10 }}>
                This is a real CAT DILR section{meta ? <> &mdash; <strong>{meta.questions} questions</strong> in <strong>{meta.duration_minutes} minutes</strong></> : ''}, with every Data-Interpretation set from that slot removed so only the real logical-reasoning sets remain.
              </li>
              <li style={{ marginBottom: 10 }}>Questions come in <strong>sets that share a scenario</strong> &mdash; several questions in a row will refer back to the same passage shown above them. Read the scenario once, then work through all the questions tied to it.</li>
              <li style={{ marginBottom: 10 }}><strong>The clock does not auto-submit.</strong> When the timer reaches zero it keeps running into negative time and the label changes to &ldquo;Overtime&rdquo;, so you can see exactly how far past the limit you went. Finish and submit whenever you are ready.</li>
              ...
              <li style={{ marginBottom: 10 }}>An unattempted question always scores <strong>0</strong>{meta ? <>. The maximum possible score is <strong>{meta.marks}</strong></> : ''}.</li>
              <li style={{ marginBottom: 10 }}>An on-screen <strong>calculator</strong> is available.</li>
            ...
            <ol start={9} style={{ paddingLeft: 24, margin: 0 }}>
              ...
              <li style={{ marginBottom: 10 }}>After you submit, you can review every question with the correct answer and a detailed worked explanation.</li>
            </ol>
          </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 12, marginTop: 16, paddingBottom: 24 }}>
            <button
              onClick={() => router.push('/dilr-tests')}
...
export default function DilrtInstructionsPage() {
  return (
    <Suspense fallback={<div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#e8e8e8', color: '#312e81', fontWeight: 'bold' }}>Loading…</div>}>
      <DilrtInstructionsInner />
    </Suspense>
  );
}
