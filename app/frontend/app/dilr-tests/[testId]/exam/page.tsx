...
const DEFAULT_TIME = 30 * 60; // placeholder until the server returns the real per-test time
...
function DilrtExamInner() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await apiFetch('/api/dilrt/start', {
          method: 'POST',
  ...
  const persist = useCallback(async () => {
    ...
    setSaveState('saving');
    try {
      const res = await apiFetch(`/api/dilrt/${sessionId}/save`, {
        method: 'POST',
  ...
  const commit = useCallback((markForReview: boolean, advance: boolean) => {
    if (!q) return;
    const ans = q.question_type === 'TITA' ? (pendingAnswer && pendingAnswer.trim() ? pendingAnswer.trim() : null) : pendingAnswer;
  ...
  const saveAndExit = useCallback(async () => {
    commit(false, false);
    setTimeout(async () => { await persist(); router.push('/dilr-tests'); }, 100);
  ...
  const doSubmit = useCallback(async () => {
    ...
    try {
      const res = await apiFetch(`/api/dilrt/${sessionId}/submit`, {
        method: 'POST',
        body: JSON.stringify({ question_states: finalStates, time_left: timeLeftRef.current }),
      });
      if (!res.ok) throw new Error('submit failed');
      router.push(`/dilr-tests/${testId}/results?sid=${sessionId}`);
    } catch {
      setSubmitting(false);
  ...
  if (loading) {
    return (
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#e8e8e8', color: '#312e81', flexDirection: 'column', gap: 16 }}>
        <div style={{ width: 48, height: 48, border: '4px solid #312e81', borderTopColor: 'transparent', borderRadius: '50%', animation: 'spin 1s linear infinite' }} />
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', background: '#d6d6d6', color: '#111111', fontFamily: 'Arial, Helvetica, sans-serif' }}>
      ...
      {/* Top Bar */}
      <div style={{ background: '#312e81', color: 'white', padding: '0 12px', display: 'flex', alignItems: 'stretch', borderBottom: '2px solid #4F46E5', minHeight: 44, flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, paddingRight: 16, borderRight: '1px solid rgba(255,255,255,0.2)' }}>
          <div style={{ width: 30, height: 30, background: '#4F46E5', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 'bold', fontSize: 13 }}>C</div>
          <div style={{ fontSize: 12, fontWeight: 'bold', lineHeight: 1.2 }}>LR Sample Test<br /><span style={{ color: '#c7c9f5', fontSize: 10, fontWeight: 'normal' }}>{label ? `${label} · ` : ''}Test {testId}</span></div>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', flex: 1, paddingLeft: 16 }}>
          <div style={{ padding: '0 16px', background: '#4F46E5', display: 'flex', alignItems: 'center', height: '100%', fontSize: 13, fontWeight: 'bold' }}>Logical Reasoning</div>
        </div>

        {/* Save indicator */}
        <div style={{ display: 'flex', alignItems: 'center', paddingRight: 14, fontSize: 11, color: '#c7c9f5' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, paddingLeft: 8 }}>
          <div style={{ textAlign: 'right' }}>
            <div style={{ fontSize: 10, color: '#c7c9f5' }}>{overtime ? 'Overtime' : 'Time Left'}</div>
      ...
      {/* Marks info bar */}
      <div style={{ background: '#eef0fe', borderBottom: '1px solid #c5c9f5', padding: '4px 16px', fontSize: 11, color: '#333', display: 'flex', gap: 20, alignItems: 'center', flexShrink: 0 }}>
      ...
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden', minHeight: 0 }}>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', minWidth: 0 }}>
          <div style={{ flex: 1, overflow: 'auto', padding: '12px 16px', background: 'white' }}>
            ...
            {passage && (
              <div style={{ background: '#f9f9f9', border: '1px solid #ddd', borderRadius: 4, padding: '12px 16px', marginBottom: 16, fontSize: 13, lineHeight: 1.8, color: '#222', maxHeight: 520, overflow: 'auto' }}>
                <div style={{ fontWeight: 'bold', fontSize: 11, color: '#312e81', marginBottom: 8, textTransform: 'uppercase', letterSpacing: 0.5 }}>Scenario</div>
                <MathText>{passage}</MathText>
              </div>
            )}

            <div style={{ marginBottom: 20 }}>
              <div style={{ fontWeight: 'bold', fontSize: 13, marginBottom: 10, color: '#312e81' }}>Question No. {currentIdx + 1}</div>
                  ...
                  {q.options.map((opt, oi) => {
                    ...
                    return (
                      <label key={oi} style={{
                        ...
                        background: selected ? '#eef0fe' : '#f9f9f9',
                        ...
                        <input
                          ...
                          style={{ marginTop: 2, accentColor: '#4F46E5', width: 16, height: 16, flexShrink: 0 }}
                        />
              ...
              {q.question_type === 'TITA' && (
                <input
                  ...
                  onChange={e => {
                    ...
                    if (/^-?[0-9]*\.?[0-9]*$/.test(v)) setPendingAnswer(v);
                  }}
                  placeholder="Type your numeric answer"
                  style={{ width: 260, padding: '10px 14px', fontSize: 15, border: '2px solid #4F46E5', borderRadius: 4, outline: 'none' }}
                />
              )}
            </div>
          </div>
          ...
          <div style={{ background: '#f0f0f0', borderTop: '2px solid #ccc', padding: '10px 16px', display: 'flex', gap: 10, alignItems: 'center', flexShrink: 0, flexWrap: 'wrap' }}>
            ...
            <button onClick={() => { commit(false, false); setShowSubmitConfirm(true); }} style={{ padding: '8px 20px', background: '#312e81', color: 'white', border: 'none', borderRadius: 4, cursor: 'pointer', fontSize: 12, fontWeight: 'bold' }}>
              Submit
            </button>
          </div>
        </div>
        ...
        {!paletteCollapsed ? (
          <div style={{ width: 220, background: '#e8e8e8', borderLeft: '2px solid #ccc', display: 'flex', flexDirection: 'column', flexShrink: 0, overflow: 'hidden' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '6px 10px', background: '#312e81', color: 'white' }}>
              <span style={{ fontSize: 12, fontWeight: 'bold' }}>Logical Reasoning</span>
            ...
            <div style={{ padding: '6px 10px', background: '#f0f0f0', borderBottom: '1px solid #ccc', fontSize: 11, fontWeight: 'bold', color: '#312e81' }}>Choose a Question</div>
            <div style={{ flex: 1, overflow: 'auto', padding: '10px' }}>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 5 }}>
                {questions.map((qq, idx) => {
                  const st = qStates[qq._id] || { status: 'not-visited', answer: null };
                  const isCurrent = idx === currentIdx;
                  const setBoundary = idx > 0 && questions[idx - 1].set_id !== qq.set_id;
                  return (
                    <button
                      key={qq._id}
                      onClick={() => goTo(idx)}
                      className={statusClass(st.status)}
                      title={setBoundary ? 'New scenario starts here' : undefined}
                      style={{
                        width: '100%', aspectRatio: '1', display: 'flex', alignItems: 'center',
                        justifyContent: 'center', borderRadius: '50%', cursor: 'pointer',
                        fontSize: 12, fontWeight: 'bold',
                        outline: isCurrent ? '3px solid #4F46E5' : (setBoundary ? '2px solid #9ca3f5' : 'none'), outlineOffset: 1,
                        border: isCurrent ? '2px solid #4F46E5' : undefined,
                      }}
                    >
                      {idx + 1}
                    </button>
          ...
          <div style={{ width: 24, background: '#312e81', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, borderLeft: '2px solid #ccc' }}>
      ...
      {showSubmitConfirm && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 2000 }}>
          <div style={{ background: 'white', borderRadius: 8, padding: 28, maxWidth: 460, width: '90%', boxShadow: '0 8px 32px rgba(0,0,0,0.3)' }}>
            ...
            <p style={{ fontSize: 13, color: '#555', margin: '0 0 16px' }}>You are about to submit {label || `Test ${testId}`}. This cannot be undone.</p>
...
export default function DilrtExamPage() {
  return (
    <Suspense fallback={
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#e8e8e8', color: '#312e81', fontWeight: 'bold' }}>Loading test…</div>
    }>
      <DilrtExamInner />
    </Suspense>
  );
