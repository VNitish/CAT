...
// Blurb per test, keyed by test number — each one is a single real CAT slot's
// DILR section with any Data-Interpretation set from that slot excluded.
const TEST_BLURB: Record<number, string> = {
  1: 'CAT 2023, Slot 1 — this slot’s entire real DILR section was already pure LR: all 4 sets, nothing excluded.',
  2: 'CAT 2021, Slot 3 — 3 of this slot’s 4 real DILR sets; the 1 Data-Interpretation set is excluded.',
};

export default function DilrTestsPage() {
  const router = useRouter();
  const { user, loading: authLoading, logout } = useAuth();
  const [tests, setTests] = useState<DilrTest[]>([]);
    ...
    <div style={{ minHeight: '100vh', background: '#FAFAFA', fontFamily: F, color: '#222222' }}>
      ...
      <nav style={{ position: 'sticky', top: 0, zIndex: 50, background: '#121212', padding: '0 40px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: 54 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          ...
          <span style={{ fontSize: 13, color: '#555555', fontFamily: F }}>LR Sample Tests</span>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <button onClick={() => router.push('/dilr')} style={{ padding: '6px 14px', background: 'transparent', color: '#666666', border: 'none', fontSize: 13, fontFamily: F, cursor: 'pointer' }}>
            LR Topics
          </button>
          <button onClick={() => router.push('/quant-tests')} style={{ padding: '6px 14px', background: 'transparent', color: '#666666', border: 'none', fontSize: 13, fontFamily: F, cursor: 'pointer' }}>
            Quant Tests
          </button>
          {user ? (
            <>
      ...
      <div style={{ borderBottom: '1px solid #e4e4e4', background: '#ffffff' }}>
        <div style={{ maxWidth: 960, margin: '0 auto', padding: '28px 40px' }}>
          ...
          <p style={{ margin: 0, fontSize: 13, color: '#777777', fontFamily: F }}>
            Full, timed LR-only sectionals &mdash; each one is a real CAT slot&apos;s actual DILR section with the Data-Interpretation sets stripped out &nbsp;&middot;&nbsp; MCQ +3 / &minus;1, TITA +3 / 0
          </p>
        </div>
      </div>
      ...
      <main style={{ maxWidth: 960, margin: '0 auto', padding: '32px 40px 80px' }}>
        ...
        {dbStatus === 'empty' && (
          <div style={{ textAlign: 'center', padding: '60px 0', color: '#aaa' }}>
            ...
            <div style={{ fontSize: 12, color: '#ccc' }}>Run <code>node api/seed_dilrt.js</code> to populate.</div>
          </div>
        )}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: 16 }}>
          {tests.sort((a, b) => a.test - b.test).map(t => (
            <div
              key={t.test}
            ...
            >
              ...
              <div style={{ padding: '16px 18px 18px' }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 10 }}>
                  <div>
                    <div style={{ fontSize: 10, fontWeight: 700, color: '#aaaaaa', letterSpacing: 0.8, textTransform: 'uppercase', marginBottom: 4, fontFamily: F }}>Real DILR Sectional</div>
                    <div style={{ fontSize: 14, fontWeight: 600, color: '#121212', letterSpacing: -0.2, fontFamily: F }}>{t.label}</div>
                  </div>
                ...
                <p style={{ margin: '0 0 14px', fontSize: 12, color: '#999', fontFamily: F, lineHeight: 1.5 }}>{TEST_BLURB[t.test] || ''}</p>
                ...
                {user ? (
                  <button
                    onClick={() => router.push(`/dilr-tests/${t.test}/instructions`)}
